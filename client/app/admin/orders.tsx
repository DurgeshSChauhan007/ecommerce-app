...
export default function AdminOrders() {
    ...
    const fetchOrders = async () => {
        try {
            ...
            const { data } = await api.get('/orders/admin/all', { headers: { Authorization: `Bearer ${token}`}})
            if (data.success) {
                setOrders(data.data)
            }
        } catch (error) {
    ...
    const updateStatus = async (newStatus: string) => {
        ...
        try {
            const token = await getToken();
            const { data } = await api.put(`/orders/${selectedOrder._id}/status`, { orderStatus: newStatus }, { headers: { Authorization: `Bearer ${token}`}})
            ...
            Alert.alert("Error", "Falied to update status")
        }
        finally {
            setUpdating(false)
        }
